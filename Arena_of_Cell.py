...
class Arena:
    def __init__(self, root_window, length=16, height=16, count_of_bots=16, start_poison=8, start_food=8, end_count=8):
        ...
        # массив ботов
        self.bots = [Bot(self.window) for i in range(self.count_of_bots)]
    ...
    def make_bots(self):
        ...
        while i < self.count_of_bots:
            ...
            if self.board[x][y].value == 0:
                ...
                self.window.ColorConfiguration(1, x, y, self.bots[i].health)  # <--
    ...
    def move_bot(self, x, y, x1, y1, bot_number):
        ...
        self.window.ColorConfiguration(0, x, y, 0)  # <--
        ...
        self.window.ColorConfiguration(1, x1, y1, self.bots[bot_number].health)  # <--
    ...
    def poison_to_food(self, x, y):
        ...
        self.window.ColorConfiguration(2, x, y, 0)  # <--
    ...
    def delete_bot(self, bot_number):
        ...
        self.window.ColorConfiguration(0, self.bots[bot_number].x, self.bots[bot_number].y, 0)  # <--
        ...
        if self.check_free() >= kol:
            ...
            while i < kol:
                ...
                if str(self.board[x][y]) == '0':
                    ...
                    i += 1
                    self.window.ColorConfiguration(obj, x, y, 0)  # <--
    ...
    def move(self):
        ...
        if self.bot_number < len(self.bots) and len(self.bots) > self.end_count:
            ...
            while not self.flag_finish:
                print('fdghjk')
                self.bots[self.bot_number].start(self, self.bot_number)
            else:
            ...
            # иначе конец поколения
                ...
                self.mutate_bots()
                for i in range(self.count_of_bots):
